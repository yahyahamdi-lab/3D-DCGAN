...
LOCAL_PATH = '/home/jerico/Documents/generative-completion/' #'/second_disk/ShapeNet/3DShapeNets/volumetric_data/'
SERVER_PATH = '/home/jerico/Documents/generative-completion/' #'/second_disk/ShapeNet/3DShapeNets/volumetric_data/'
...
def plotFromVF(vertices, faces):
    ...
    voxel_mesh = mesh.Mesh(  input_vec.vectors)
    voxel_mesh.save('voxel_mesh.stl')
...
def plotMeshFromVoxels(voxels, threshold=1.9):
    verts,faces = getVFByMarchingCubes(voxels, threshold)
...
# Save the mesh to an STL file
    voxel_mesh.save('voxel_meshs290.stl')
    #plotFromVF(v,f)
...
if __name__ == '__main__':
    ...
    file_path= 'E:/Oto3D/generative-3D-mesh-completion-master/new project/290.npy'
    epoch = 1
    i = 1
    voxels = np.load(file_path)
    voxels_smoothed = gaussian_filter(voxels.astype(float), sigma=1.3)
    plotMeshFromVoxels(voxels_smoothed, threshold=0.9)
